...
const DynamicForm = ({
    ...
    const [images, setImages] = useState(() => {
        ...
        // Добавляем mainImage если есть
        if (initialData.mainImage) {
            let imageUrl = initialData.mainImage;
            let originalUrl = initialData.mainImage;
        ...
        if (initialData.images && Array.isArray(initialData.images)) {
            initialData.images.forEach((img, idx) => {
                // Пропускаем если это тот же URL что и mainImage
                if (initialData.mainImage === img) return;
                ...
                <ImageUploader
                    ...
                    onRefresh={() => {
                        ...
                        if (type === 'property' && formData._id) {
                            // Можно сделать дополнительный запрос для обновления данных
                        }
                    }}
                />
            </div>
